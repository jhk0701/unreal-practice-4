...
void UUIStatusBar::Bind(ATDRPGPlayer* InPlayer)
{
	Player = InPlayer;
	...
	Data->Stat[EStatus::Hp]->OnValueChanged.AddUObject(this, &UUIStatusBar::OnHpChanged);
	Data->Stat[EStatus::Mp]->OnValueChanged.AddUObject(this, &UUIStatusBar::OnMpChanged);
...
void UUIStatusBar::OnHpChanged(uint32 Max, uint32 Current)
{
...
void UUIStatusBar::OnMpChanged(uint32 Max, uint32 Current)
{
	float Percent = Current > 0 ? Current / (float)Max : 0;
